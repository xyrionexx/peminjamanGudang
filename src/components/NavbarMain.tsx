...
export default function MainNavbar() {
  ...
  const handleSearchOnChange = (searchValue: string): void => {
    const huruf: string = searchValue[0].toLowerCase();
    const daerahSearchBarang: FoundBarang[] | undefined = mappedDataBarang.get(huruf);

    if (daerahSearchBarang == null) return;

    const hasil: FoundBarang[] | null = B_Search(daerahSearchBarang, searchValue);
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 bg-white shadow-md">
      <div className="navbar flex items-center py-4 px-10 gap-5">
        {/* LOGO */}
        <div>
          <Link href={'/daftarBarang'} className="cursor-pointer">
