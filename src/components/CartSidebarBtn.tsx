...
export default function CartSidebarBtn(): JSX.Element {
  ...
  const getBarangCart = () => {
    try {
      const item: FoundBarang[] | null = JSON.parse(localStorage.getItem('cart') ?? '[]');
  ...
  const handleSearch = (searchvalue: string): void => {
    const result: FoundBarang[] | null = B_Search(barang as DataBarangType[], searchvalue);
  ...
  return (
    <Sheet>
      {/* TRIGGER BUTTON */}
      <Tooltip>
        <TooltipTrigger asChild className="cursor-pointer" onMouseEnter={getBarangCart}>
          <SheetTrigger asChild>
            <Icon icon="vaadin:cart-o" width="20" height="20" className="cursor-pointer" />
          </SheetTrigger>
        </TooltipTrigger>
      ...
      <SheetContent className="flex flex-col h-full">
        <SheetHeader className="shrink-0">
          <SheetTitle className="text-green-500 text-2xl flex items-center gap-2">
            Keranjang
            <Icon icon="vaadin:cart-o" width="25" height="25" />
          </SheetTitle>
        ...
        <div className="flex-1 min-h-0 flex mx-2 flex-col gap-5">
          ...
          <div className="shrink-0">
            <EnhancedSearch onSearch={handleSearch} onReset={handleReset} />
          </div>
